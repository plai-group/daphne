...
(defn code->hy [code]
  (let [_ (println "Compiling graphical model.")
        prog (fc/code->graph code)
        _ (println "Graphical model compiled.")
        [rho G E] prog
        dim-condition (count (:Y G))
        dim-latent (- (count (:V G)) dim-condition)
        self-adjacency (for [i (range dim-latent)] [i i])
        _ (println "Inverting graphical model adjacency structure.")
        faithful-adjacency (concat (faithful-adjacency-list code) self-adjacency)
        _ (println "Adjacency structure inverted.")
        _ (println "Sampling random inverse graph.")
        rand-adjacency (concat (create-rand-adjacency (count faithful-adjacency)
                                                      dim-latent
                                                      dim-condition)
                               self-adjacency)
        _ (println "Random inverse graph with same number of edges found.")
        _ (println "Emitting hy-lang code.")]

    (template
     ;; hy-lang syntactic embedding
     ;; chose immutable Object for scoping
     (defclass FlowModel []
       "Autogenerated by daphne, do not edit. "
